...
import {
  BookingTicket,
  ListTicketRoom,
} from "../store/bookingMovie/thunkAction";
...
const MovieBooking = () => {
  ...
  const { listRoom, isLoading, listChairBooking, isBooking } = useSelector(
    (state) => state.bookingMovie
  );
  const movieInfo = listRoom.thongTinPhim;

  useEffect(() => {
    dispatch(ListTicketRoom(`?MaLichChieu=${params.showtimeId}`));
  }, [dispatch, params, isBooking]);
  ...
  const handleBookingChair = (name, type, price, codeChair) => {
    dispatch(
      ADD_CHAIR({
        id: `${name}-${type}`,
        numChair: name,
  ...
  const handleBookingTicket = () => {
    dispatch(
      BookingTicket({
        ...
        danhSachVe: listChairBooking?.map((item) => {
          return {
  ...
  const typeChair = (type, bookState, name, price, codeChair) => {
    ...
    if (type === "Thuong" && !bookState) {
      return (
        <button
          onClick={() => {
            handleBookingChair(name, type, price, codeChair);
          }}
          type="button"
    ...
    return (
      <button
        type="button"
        className="text-red-500 bg-white cursor-not-allowed font-medium rounded-lg text-sm px-4 py-3 text-center"
        disabled
      >
        X
      </button>
          ...
          <div className="basis-3/5">
            ...
            <div className="flex flex-wrap gap-2 px-2 py-10">
              ...
              <div className="flex flex-wrap gap-2 items-center pr-4">
                <button
                  type="button"
                  className="text-red-500 bg-white cursor-not-allowed font-medium rounded-lg text-sm px-2.5 py-1 text-center"
                  disabled
                >
                  X
                </button>
            ...
            <div className="flex flex-wrap py-10 border-b-2 border-b-gray-400">
              ...
              <div className="flex flex-wrap gap-2 basis-2/5 justify-end">
                {listChairBooking?.map((item) => {
                  return (
                    <p
                      className=" text-gray-400 text-sm text-end"
                      key={item.numChair}
                    >
